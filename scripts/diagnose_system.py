...
def check_python_packages():
    ...
    print(f"[1/4] Checking Python Packages...")
    for pkg in REQUIRED_PKGS:
        ...
        if spec is None:
            REPORT["CRITICAL"].append(f"Missing Python Package: '{pkg}' (Run: pip install {pkg})")

def check_system_binaries():
    """Phase 2: Check if Typst and Git are in PATH."""
    print(f"[2/4] Checking System Tools...")
    for tool in REQUIRED_BINS:
        if shutil.which(tool) is None:
            REPORT["CRITICAL"].append(f"Missing System Tool: '{tool}' (Install via 'winget install {tool}')")

def check_data_integrity():
    """Phase 3: Validate YAML files without crashing."""
    print(f"[3/4] Validating Data Integrity...")
        ...
        try:
            with open(f, "r", encoding="utf-8") as stream:
                ...
                if content is None:
                    ...
                    REPORT["DATA_CORRUPTION"].append(f"{f.name}: Invalid structure (must be list or dict)")
        except yaml.YAMLError as e:
            # We catch the specific syntax error line
            if hasattr(e, 'problem_mark'):
                mark = e.problem_mark
                REPORT["DATA_CORRUPTION"].append(f"{f.name}: Syntax Error at line {mark.line+1}, col {mark.column+1}")
            else:
...
def check_compilation_dry_run():
    ...
    print(f"[4/4] Testing Compilation Engine...")
    ...
    try:
        ...
        if result.returncode != 0:
            REPORT["CRITICAL"].append(f"Typst Compilation Failed:\n{result.stderr.strip()}")
    except Exception as e:
